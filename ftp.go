...
func ConnectDbg(addr string) (*FTP, error) {
	...
	return UseConnectionDbg(addr, conn)
}

// Use existing connection to build ftp object, debug is OFF
...
// Use existing connection to build ftp object, debug is ON
func UseConnectionDbg(addr string, conn net.Conn) (*FTP, error) {
	writer := bufio.NewWriter(conn)
	reader := bufio.NewReader(conn)
